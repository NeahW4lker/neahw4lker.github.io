# Hello World, moi c'est Haritiana 👋

Je suis un **Ingénieur Cybersécurité** passionné, avec de solides fondations en **Développement Web**. J'aime lier le code et la sécurité pour concevoir des environnements résilients et relever des défis techniques stimulants.

- 🛡️ Dernièrement **Deputy CISO & IT Infra Project Officer**
- 🎓 Diplômé du **Conservatoire National des Arts et Métiers**
- 💼 Compétences clés : Gouvernance & Risques (ISO 27001, GDPR), VAPT, Réseau, Dev JavaScript/Linux.
- 🌍 Langues : Français, Anglais, Malagasy, Allemand (B1)

---

### 🛠️ Mes Technologies & Outils

- **Sécurité & GRC :** ISO 27001, Risk Management, Pentesting, Incident Response
- **Développement :** JavaScript, Web Tools, Scripting Linux / Bash
- **Systèmes :** Linux System Administration, Docker

---

### 🚀 Projets Phares (Sélection)

1. **ServGuardAI** - Outil d'automatisation de conformité / durcissement boosté avec de l'AI pour les serveurs linux avec un côut fortement réduit.
2. **SOC 100% Opensource et automatisées** - Mise en place d'un SOC 100% opensource et automatisées en utilisant Wazuh - Tracecat/n8n - GLPI.

---

### 📜 Certifications

- **CompTIA Pentest+** (Skillsoft)
- **CompTIA Cybersecurity Analyst+** (Vulnerability & Penetration Testing)
- **Ethical Hacker & CISSP** modules (Security Governance)

---

📫 **Pour me contacter :**
- 📧 Email : hart.tsiry@gmail.com
- 🤳Phone : +261 34 75 058 87
